...
- **`promptcanary init --test-runner`** — scaffold a Vitest or Jest test file alongside the YAML config
- **Composite GitHub Action** — `.github/actions/run/` for running prompt tests in CI with minimal config
- **Comparison page** — honest feature comparison between PromptCanary and promptfoo
- **Jest getting-started guide** — ESM-compatible Jest setup example alongside existing Vitest docs
- **Example project** — `examples/chatbot-app/` with realistic prompt tests demonstrating all features
